...
import com.card.note.IntegrationTest;
import com.card.note.config.EmbeddedKafka;
...
@WithMockUser
@EmbeddedKafka
class CardNoteBackendKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/card-note-backend-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/card-note-backend-kafka/consume")
