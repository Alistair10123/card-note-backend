package com.card.note.web.rest;

import com.card.note.config.KafkaSseConsumer;
import com.card.note.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/card-note-backend-kafka")
public class CardNoteBackendKafkaResource {

    private final Logger log = LoggerFactory.getLogger(CardNoteBackendKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public CardNoteBackendKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
